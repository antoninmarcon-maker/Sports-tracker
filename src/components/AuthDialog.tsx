...
export function AuthDialog({ open, onOpenChange, onGuest, message }: AuthDialogProps) {
  ...
  const handleApple = async () => {
    ...
    if (error) toast.error('Erreur Apple : ' + (error as any).message);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm rounded-2xl">
        ...
        {message && (
          <p className="text-xs text-center text-muted-foreground bg-secondary/50 rounded-lg p-2">{message}</p>
        )}
        ...
        <div className="space-y-3">
          ...
          {mode !== 'forgot' && (
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg bg-card border border-border text-foreground text-sm font-medium hover:bg-secondary transition-all"
              >
          ...
          {mode === 'login' && (
            <button onClick={() => setMode('forgot')} className="text-xs text-primary hover:underline w-full text-right">
              Mot de passe oublié ?
            </button>
          )}
          ...
          >
            {loading ? '...' : mode === 'forgot' ? 'Envoyer le lien' : mode === 'signup' ? 'Créer le compte' : 'Se connecter'}
          </button>
          ...
          {mode === 'forgot' && (
            <button onClick={() => setMode('login')} className="w-full text-xs text-muted-foreground hover:text-foreground text-center">
          ...
          {onGuest && mode !== 'forgot' && (
            <>
              ...
              <button
                onClick={() => { onGuest(); onOpenChange(false); }}
                className="w-full py-2.5 rounded-lg bg-secondary text-secondary-foreground font-semibold text-sm hover:bg-secondary/80 transition-all"
              >
